# 2026-01-21 - Step Plan V5（Drift Hardening：SITE_URL SSoT + Home Event CTA URL + Settings Cleanup）(Archive)
...
- What shipped: PR-19..PR-21（SITE_URL SSoT hardening for OpenRouter；Home「講座邀請」CTA URL allowlist validation；集中 `company_settings` lookup helper）
...
- 原 `doc/meta/STEP_PLAN.md` 標題誤用 V4（與既有 V4 snapshot 重號）；此檔以時間序列歸檔為 V5，snapshot 內容保持 verbatim。
