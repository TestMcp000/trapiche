...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const title =
    locale === 'zh' ? `${category} - 商城` : `${category} - Shop`;
  const description =
    locale === 'zh'
      ? `瀏覽 ${category} 分類的商品`
      : `Browse products in ${category} category`;

  return {
    title,
    description,
    alternates: getMetadataAlternates(`/shop/${category}`, locale),
    openGraph: {
      title,
      description,
      type: 'website',
...
export default async function ShopCategoryPage({
  ...
  const productsResult = await getVisibleProductsCached({
    category,
    search: query.q,
    sort:
      (query.sort as 'newest' | 'popular' | 'price-asc' | 'price-desc') ||
      'newest',
    limit: 24,
    offset: 0,
  });
        ...
        {categories.length > 0 && (
          <nav className="mb-8">
            <div className="flex flex-wrap gap-2">
              <Link
                href={`/${locale}/shop`}
                className="px-4 py-2 rounded-full text-sm font-medium transition-colors bg-surface-raised text-secondary hover:text-primary"
              >
                {allLabel}
              </Link>
              {categories.map((cat) => (
